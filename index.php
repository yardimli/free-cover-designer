						...
						if (isset($design_data['layers']) && is_array($design_data['layers'])) {
							foreach ($design_data['layers'] as $layer) {
								if (isset($layer['type']) && $layer['type'] === 'text' && !empty($layer['fontFamily'])) {
									...
									if (!in_array(strtolower($font_family), $known_local) && !is_numeric($font_family) && preg_match('/^[a-zA-Z0-9\s]+$/', $font_family)) {
										// Assume it might be a Google Font if not obviously local/generic and looks like a name
	...
	if (is_dir($covers_dir)) {
		...
		foreach ($files as $file) {
			...
			if (str_ends_with(strtolower($file), '-preview.jpg')) {
				...
				// Prefer JPG, then PNG for target
				...
				if ($target_image_path) {
					$covers_data[] = [
						...
						'imagePath' => $target_image_path // Path to the target image
					];
				}
			}
		}
...
<head>
	...
	<?php if (!empty($google_fonts_used)): ?>
		...
		// Request common weights/styles plus italics (adjust if needed)
		$font_families_params = [];
		foreach ($google_fonts_used as $font) {
			// Ensure URL encoding for font names with spaces
			$encoded_font = urlencode($font);
			// Request a range of weights and italic styles
			$font_families_params[] = "family={$encoded_font}:ital,wght@0,300;0,400;0,700;0,900;1,300;1,400;1,700;1,900";
		}
		$google_font_url = "https://fonts.googleapis.com/css2?" . implode('&', $font_families_params) . "&display=swap";
		?>
		<link href="<?php echo htmlspecialchars($google_font_url); ?>" rel="stylesheet">
	<?php endif; ?>
...
<script id="googleFontsData" type="application/json"><?php echo $google_fonts_list_json; ?></script>
